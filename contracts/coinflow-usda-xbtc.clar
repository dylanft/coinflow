...
(define-constant ERR-SENDING-TO-WRONG-LENDER (err u105))
(define-constant ERR-NO-EXISTING-LOAN-OFFER-FROM-LENDER (err u106))
...
(define-constant ERR-LENDER-HAS-MORE-TIME-TO-RETURN-COLLATERAL (err u113))
(define-constant ERR-LENDER-ALREADY-RETURNED-COLLATERAL (err u114))
(define-constant ERR-LOAN-AMT-INCORRECT (err u115))
(define-constant UNAUTHORIZED-MVMT-BORROWER-COLLATERAL (err u116))
(define-constant ERR-LOAN-AMOUNT-MISMATCH (err u117))
...
(define-constant LENDER-COLLATERAL-REPAYMENT-BUFFER u100)
...
(define-map initDeadline principal uint) ;; borrower must send collateral to lender by initDeadline. principal = lender.
...
(define-map lenderCollateralAmount 
        {borrower: principal, lender: principal}
        uint
)
...
(define-read-only (get-loan-health (borrower principal) (lender principal))
    (map-get? loans {borrower: borrower, lender: lender})
)

(define-read-only (get-loan-balance (borrower principal) (lender principal))
    (default-to u0 (map-get? loanBalances {borrower: borrower, lender: lender}))
)

(define-read-only (get-lender-collateral-amount (borrower principal) (lender principal))
	(map-get? lenderCollateralAmount 
        {borrower: borrower, lender: lender}
    )
)

(define-read-only (get-status-of-lender-collateral-return (borrower principal) (lender principal))
    (default-to false (map-get? hasLenderReturnedCollateral {borrower: borrower, lender: lender}))
)

(define-private (set-status-of-lender-collateral-return (borrower principal) (lender principal) (status bool))
...
(define-private (set-lender-collateral-amount (amount uint) (borrower principal) (lender principal))
	(map-set lenderCollateralAmount 
        {borrower: borrower, lender: lender}
        amount
    )
)
...
(define-public (lock-up-lender-collateral (borrower principal) (blocksToInitialize uint) (loanAmount uint))
	;; TODO: assert that there is interest from the borrower
	;; TODO: lookup loanAmount and ensure lender has enough collateral to lock up and to offer the loan
	(begin 
        ...
        (set-lender-collateral-amount (* u4 loanAmount) borrower tx-sender)
		(ok true)
	)
)

(define-public (initialize-usda-loan (usdaAmount uint) (xbtcAmount uint) (borrower principal) (lender principal) (memo (optional (buff 34))))
    (begin
        ;; asserts that the borrower has declared interest in getting a loan of some amount > u0.
        (asserts! (> (unwrap! (get-loan-interest-from borrower) ERR-NO-INTEREST) u0) ERR-NO-INTEREST)

        ;; asserts that the usdaAmount requested from this txn matches what the original loan interest was
        (asserts! (is-eq usdaAmount (unwrap! (get-loan-interest-from borrower) ERR-NO-INTEREST)) ERR-LOAN-AMOUNT-MISMATCH)

		;; asserts that lender input is same as wallet that locked up the loan amount and lender collateral.
		(asserts! (is-eq lender (unwrap! (get-loan-source-from tx-sender) ERR-NO-EXISTING-LOAN-OFFER-FROM-LENDER)) ERR-SENDING-TO-WRONG-LENDER)
		...
		;; Borrower sends collateral for their loan amount to the lender
        ...
        (map-set loans
            {borrower: tx-sender, lender: lender}
            {
                ...
                ;; lenderCollateralAmt: (* u4 (unwrap! (get-loan-interest-from tx-sender) ERR-NO-INTEREST))
                lenderCollateralAmt: (unwrap! (get-lender-collateral-amount tx-sender lender) ERR-NO-EXISTING-LOAN-OFFER-FROM-LENDER) ;; in USDA
            }
        )
        ...
        ;; reset the loan interest to zero for the borrower
        (map-delete loanInterest tx-sender)
		(map-delete loanLength tx-sender)

        (set-status-of-lender-collateral-return borrower lender false)
...
;; assumptions: 
;;      if don't payback in full, cannot claim any of borrower lenders. in practice, this should be prorated to some extent if borrower pays back 99% for example.
;;      who is able to call this txn and when? need to be sure!
(define-public (claim-lender-collateral (borrower principal) (lender principal))
    (begin
		...
		;; (try! (as-contract (stx-transfer? (unwrap-panic (get-lender-collateral-amount borrower lender)) tx-sender lender)))
		(try! (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.usda-token transfer (unwrap-panic (get-lender-collateral-amount borrower lender)) tx-sender lender none)))
...
(define-public (return-collateral-to-borrower (amount uint) (borrower principal) (lender principal) (memo (optional (buff 34))))
    (begin
        ...
        (set-status-of-lender-collateral-return borrower lender true)
        (ok true)
    )
)
...
(define-public (claim-borrower-collateral (amount uint) (borrower principal) (lender principal))
    (begin
        ...
        (asserts! (is-eq amount (get borrowerCollateralAmt (unwrap-panic (get-loan-health borrower lender)))) ERR-UNEQUAL-COLLATERAL-REPAYMENT)
        ;; (asserts! (>= (+ block-height LENDER-COLLATERAL-REPAYMENT-BUFFER) (get repaymentBlockDeadline (unwrap-panic (get-loan-health borrower lender)))) ERR-LENDER-HAS-MORE-TIME-TO-RETURN-COLLATERAL)

        ;; Ensures borrower can ONLY claim the LENDERs collateral if the LENDER has NOT returned the borrowers collateral by the repayment deadline + some buffer of blocks
        (asserts! (is-eq false (get-status-of-lender-collateral-return borrower lender)) ERR-LENDER-ALREADY-RETURNED-COLLATERAL)
        (asserts! (>= block-height (+ LENDER-COLLATERAL-REPAYMENT-BUFFER (get repaymentBlockDeadline (unwrap-panic (get-loan-health borrower lender))))) ERR-LENDER-HAS-MORE-TIME-TO-RETURN-COLLATERAL)
...
;;TODO: create a function for the borrower to claim the lender collateral IFF borrower repays in full AND lender doesn't return collateral

;; TODO: create a dictionary like data-var to store all loans numbered 1 through n and implement in appropriate functions
;; aybe appropriate functions is just send-STX-collateral-to-lender?
;; then reset the variables associated with the wallets after loan initialized?
;; or better to reset some after lender collateral locked up and rest after borrowers collateral sent to lender


;; TODO: Write a function for lender to claim the lender collateral
;; must check if loan initialized or not
;; must check appropriate deadlineese depending on initialized status

;; TODO: introduce interest rate in a way where borrower can choose from best lender option
;; where each lender sets their own time limit on the offer in num of blocks
...
;; (contract-call? 'ST1HTBVD3JG9C05J7HBJTHGR0GGW7KXW28M5JS8QE.coinflow send-collateral-to-lender u500 'ST1J4G6RR643BCG8G8SR6M2D9Z9KXT2NJDRK3FBTK)
...
;; (contract-call? 'ST1HTBVD3JG9C05J7HBJTHGR0GGW7KXW28M5JS8QE.coinflow lock-up-lender-collateral 'ST1J4G6RR643BCG8G8SR6M2D9Z9KXT2NJDRK3FBTK u288 u1000)
;; ::get_assets_maps
...
;; ::advance_chain_tip 1
;; ::set_tx_sender ST1J4G6RR643BCG8G8SR6M2D9Z9KXT2NJDRK3FBTK
;; (contract-call? 'ST1HTBVD3JG9C05J7HBJTHGR0GGW7KXW28M5JS8QE.coinflow send-collateral-to-lender u2000 'STFCVYY1RJDNJHST7RRTPACYHVJQDJ7R1DWTQHQA)
